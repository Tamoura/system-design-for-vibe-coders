...
export const POST = publicApi('monitors:write', async (req, api) => {
  ...
  return idempotent(req, api.orgId, raw, async () => {
    ...
    const monitor = await createMonitor({ orgId: api.orgId, userId: api.createdBy, audit: api.audit }, { name: input.name, url: input.url, intervalSeconds: input.interval_seconds }, 'api');

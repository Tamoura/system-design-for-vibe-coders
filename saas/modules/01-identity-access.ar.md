...
**العزل على نطاق واسع.** مع آلاف المستأجرين، تصبح الأسئلة: أين تعيش بيانات المستأجر (جداول مشتركة، أو مخطط لكل مستأجر (schema per tenant)، أو قاعدة بيانات لكل مستأجر)، وكيف تمنع مستأجرًا ضخمًا واحدًا من إبطاء الجميع، وكيف تلتزم بطلب "بياناتنا تبقى في الاتحاد الأوروبي". هذه موضوعات الدرس 2.4. والخبر الجيد أن وجود `org_id` على كل صف يبقي كل الخيارات مفتوحة: فمثلًا، يوزّع Citus (shards) جداول Postgres حسب عمود توزيع (distribution column)، ومعرّف المستأجر هو الاختيار النموذجي.

## 🏆 أفضل المستودعات

| المستودع | ما هو | التقنيات | الترخيص | اختره عندما |
|---|---|---|---|---|
| [nextjs/saas-starter](https://github.com/nextjs/saas-starter) | قالب SaaS رسمي (template) ومبسّط لـ Next.js فيه فرق ودعوات وأدوار وسجل نشاط | TypeScript (activity log), Drizzle, Postgres | MIT | تريد أصغر نسخة مقروءة من الهيكل كاملًا |
...
| [citusdata/citus](https://github.com/citusdata/citus) | إضافة لـ Postgres (extension) توزّع الجداول على العقد (nodes) حسب عمود | C | AGPL-3.0 | تخطط لنطاق ضخم جدًا من المستأجرين وتريد أن ترى لماذا يهم وجود `org_id` في كل مكان |
...
- **اشترِ** عندما تستخدم أصلًا مزوّد مصادقة مُدارًا (managed auth provider) يتضمن المنظمات. Clerk وWorkOS وAuth0 (Organizations) وStytch وKinde كلها تمثّل المنظمات والعضويات والدعوات. هذا سريع، لكن أهم علاقة تجارية (business relation) لديك تعيش الآن في قاعدة بيانات المزوّد، لذلك انسخ المنظمات (mirror) والعضويات إلى جداولك عبر الويب هوك (Webhook).
- **استضف بنفسك** خادم هوية فيه نموذج منظمات (identity server) (Logto، Zitadel، منظمات Keycloak) إذا كنت تشغّل واحدًا أصلًا للمصادقة.

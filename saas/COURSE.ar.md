...
**الاستضافة الذاتية.** إن كنت تقدّم نسخة قابلة للاستضافة الذاتية (الدرس 7.4) أو تحتاج تخزينًا داخل مقر العميل، فأنت تحتاج خادمًا متوافقًا مع S3. كان MinIO الخيار الافتراضي لسنوات، لكن في 2025 اتجهت نسخته المجتمعية نحو التوزيع كمصدر فقط ووضع الصيانة، لذلك تحقق من وضعه الحالي قبل اعتماده. SeaweedFS وGarage (خادم S3 خفيف وموزّع جغرافيًا من Deuxfleurs) هما البديلان الأكثر ذكرًا.
...
Garage ليس على GitHub؛ تجده على https://garagehq.deuxfleurs.fr. وهو مناسب لعمليات النشر الصغيرة المستضافة ذاتيًا أو الموزعة على عدة مواقع.
...
**Cal.com (`calcom/cal.com`).** ابحث في مخطط Prisma عن `teamId` و`organizationId` لترى نموذج مجمّع نمت فوق فرقه طبقة "organizations"، وكيف تُحصر قيود التفرّد.
...
**calcom/cal.com** — API عام كبير مع مفاتيح لكل مستخدم أو فريق. وقت كتابة هذا الدرس، تعيش تطبيقات الـ API تحت `apps/api`. ابحث عن `apiKey` ودوال مساعدة على غرار `hashAPIKey`، وانظر كيف تُدار إصدارات الـ API جنبًا إلى جنب مع تطور الـ API.
...
**calcom/cal.com** — ويب هوك ومتجر تطبيقات في مستودع واحد. ابحث في مخطط Prisma عن `Webhook` وعن تعداد أحداث التشغيل لترى الاشتراكات في أنواع الأحداث لكل مستخدم أو فريق، ثم ابحث عن `sendPayload` أو `webhook` في حزم الميزات لتجد المرسل. وللتكاملات، وقت كتابة هذا الدرس، يحتوي `packages/app-store` على مجلد لكل تطبيق؛ افتح اثنين (مثلًا تطبيق فيديو وتطبيق تقويم) وقارن بنيتيهما.
...
**calcom/cal.com** — منشئ سير عمل للتذكيرات يراه المستخدم. ابحث عن `workflow` و`WorkflowStep` في مخطط Prisma لترى كيف تُخزَّن المشغّلات التي يعرّفها العميل ("قبل بدء الحدث") والإزاحات والإجراءات (بريد، SMS) كبيانات، ثم ابحث عن المكان الذي تُجدوَل فيه التذكيرات وتُلغى عندما تتغير الحجوزات. لاحظ كيف يجب أن تؤدي إعادة جدولة حجز إلى تحديث الخطوات المجدولة أصلًا.

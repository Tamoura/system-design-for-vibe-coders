...
*الطبقة التي تبقى بعد كل إعادة كتابة (rewrite). سيُستبدل كودك (code)، أما بياناتك (data) فيجب أن تنجو من
...
بدا الفهرس (catalog) كاملًا. كان لكل عنصرٍ صفحةٌ نظيفة وعنوانٌ ثابت وتحميلٌ سريع. ثم رفع أحد
المنتجين نسخةً ثانية من إصدارٍ سبق أن نشره — الإصدار نفسه بقراءةٍ مختلفة — فاختفت
إحدى النسختين بهدوء. لا خطأ (error) ولا انهيار (crash). «نجح» الرفع (upload). لكن العنصر لم يكن هناك بعده،
أو اختفى القديم. وأيُّهما نجا بدا معتمدًا على توقيتٍ لم يستطع أحدٌ تفسيره.

كانت الفرضيات الأولى كلها عن كود (code) الرفع: خطأٌ في الحفظ، أو مشكلة كاش (Cache)، أو
تسابق (Race). حدّق المهندسون في مسار الكتابة (write path) ساعات. وكان مسار الكتابة سليمًا.

المشكلة الحقيقية اتُّخذت قبل أشهر، في مخططٍ (Schema) لم يظنه أحدٌ خطِرًا. كان كل
عنصرٍ في الفهرس **مفتاحُه الإصدار (Edition)** — عُومل الإصدار بوصفه هويةَ (identity) الشيء،
أي اسمه الفريد في قاعدة البيانات (Database). ونجح ذلك تمامًا حتى اليوم الذي أنتج
فيه المجالُ (domain) شيئين مختلفين يتشاركان إصدارًا واحدًا. كانت طبقة التخزين (storage layout) تحته تستطيع
حفظ النسختين، لكن *نموذج البيانات (data model)* لم يملك وسيلةً للتمييز بينهما. شيئان، وهويةٌ
واحدة — ففعلت قاعدة البيانات ما أمرتها به بالضبط: عاملت الثاني بوصفه إعادةَ صياغةٍ
للأول، فابتلع أحدهما الآخر.

**المفاتيح التي تختارها مبكرًا هي القيود (constraints) التي تعيش معها أطول.** قاعدة البيانات نفسها
...
المخطط (Schema) هو شكل بياناتك: ما هو «المستخدم»، وما هو «المنشور»، وأي الحقول (fields)
موجودة، والجزء الحامل: ما الذي يجعل كل سجلٍّ (record) **فريدًا**. والمفتاح (Key) — أو
المفتاح الأساسي (Primary Key) — هو الحقل (field)، أو تركيبةُ حقول، التي تستعملها قاعدة
البيانات لتقول: *هذا السجل وذاك الشيءُ نفسُه، أو ليسا كذلك.*

إن أخطأت المفتاح ورث كلُّ سطرٍ صحيحٍ آخر الخطأ. لم تكن الحادثة أعلاه خطأً في دالة (function)،
بل خطأً في *تعريف*. وهذه لا تظهر في اختباراتٍ (tests) كُتبت قبل وجود الحالة الحدّية.
...
| | ما هو | الفخ |
|---|---|---|
| **مفتاح طبيعي (Natural Key)** | قيمة من العالم الواقعي «يُفترض» أنها فريدة (إصدار، بريد، ISBN، اسم مختصر) | العالم الواقعي ينتج في النهاية تكرارًا (duplicate) أو تغييرًا أقسمت أنه مستحيل |
| **مفتاح بديل (Surrogate Key)** | معرّف (ID) بلا معنى يولّده النظام (قيمة فريدة عشوائية أو تسلسلية) | لا فخ في الهوية — لكن يبقى عليك تحديد أي تفرّدٍ *تجاري* تفرضه فوقه |
...
مملٌّ وصغير، وصمد أكثر من عقد. تصميم الهوية تصميمُ أنظمةٍ (system design) حقيقي، وأفضل الأجوبة أبسطها.
...
المفهوم جيدًا وأعطه مخططًا صحيحًا.** قاعدة بياناتٍ علائقيةٍ (relational database) بمفاتيح صحيحة ستحمل أي
...
ملايين الصفوف (rows) وعشرات مسارات الكود (code paths) الهويةَ القديمة — فهو المكلف، وأحيانًا ترحيلٌ
...
أنت تصمّم نموذج بيانات **Relay** — الحسابات (accounts)، ملفات المنتجين، الرفوعات، الخلاصات
(Feeds). أنجز تفكير الهوية *قبل* أن يولّد الوكيل (agent) الجداول (tables)، لأنه الجزء المكلف إعادته.

1. **اسرد الأسماء وهوياتها.**
   > *«اسرد كل كيانٍ (entity) أساسي في Relay — مستخدم، منتِج، رفع، إصدار، نسخة، عنصر خلاصة.
...
الختام: *«أودع (commit) برسالة `02-1-relay-schema-identity`.»*

> 🔧 **تحت الغطاء** (اختياري): إصلاح «البُعد الثاني» فهرسٌ فريدٌ مركّب (composite unique index) — في SQL:
...
حذف مسؤولٌ (admin) ملفَ وسائطٍ من تخزين الكائنات (Object Storage) في المنتج. اختفى —
تأكّد، فُحص، غائب. وبعد يومٍ أو اثنين عاد. احذفه ثانيةً فيعود ثانية. تصرّف كأنه
مسٌّ لا كأنه خطأ.

كانت الفرضيات كلها عن كود الحذف (delete): ربما لا يحذف فعلًا، أو كاشٌ (cache) يقدّم قائمةً قديمة، أو
...
تكرارٍ عند الطلب (pull-through replication bridge)** (ميزةٌ يسمّيها R2 اسم Sippy): حين يُطلب من R2 ملفٌ لا يملكه،
يجلبه بهدوء من مستودع GCS (bucket) القديم ويحتفظ بنسخة. وهذا بالضبط ما تريده *أثناء* الترحيل
— فلا يصيب مستخدمٌ ملفًا مفقودًا. لكنه بقي مشغّلًا *بعد* الترحيل. فالتسلسل كان:
المسؤول يحذف الملف من R2 ← الطلب التالي (request) يجد R2 يفتقده ← يسحبه R2 من GCS الذي ما زال
...
المصدرَين (dual-source phase) صراحةً. **الترحيل لا ينتهي حين يعمل النظام الجديد، بل حين يعجز النظام
القديم عن الفعل.**
...
كلٌّ منها بمفتاح (أي مسار). ليست قاعدة بياناتٍ (database) ولا مجلدًا على خادمك (server). يخزّن تطبيقك
*مرجعًا (reference)* (المفتاح/الرابط (key/URL))، وتعيش البايتات (bytes) في مخزن الكائنات، وتُقدَّم للمستخدمين
...
المزدوجة (تقرأ من أيٍّ منهما)، وجسور السحب عند الطلب (pull-through bridges) — كلها مالكان لحقيقةٍ واحدة،
...
الطريقة الآمنة لنقل بيانات نظامٍ حيّ أطوارٌ صريحةٌ **بمعايير خروجٍ (exit criteria) مكتوبة** — لا
...
| كتابة مزدوجة (dual-write) | القديم (للقراءة) | كل كتابةٍ جديدة تصل للاثنين؛ اكتمل نقل (backfill) بيانات القديم |
| قراءة مزدوجة (dual-read) + تحقّق | الجديد (للقراءة)، القديم شبكةَ أمان (safety net) | الجديد يطابق القديم على عينةٍ متحقَّقة؛ الحذف يعمل من الطرف للطرف (end-to-end) |
| **الجديد فقط** | النظام الجديد | **أُطفئ القديم — الجسر مغلق، الصلاحيات مُلغاة (credentials revoked)** |
...
توقفاتُ جمع مهملات (garbage-collection pauses) — لا بموضة، وكلُّ نقلةٍ تُحمَل إلى الاكتمال مع تقاعد المخزن
...
4. **اكتب جدول أطوار الترحيل.**
   > *«أنشئ دليل ترحيلٍ (migration runbook) لتخزين Relay بالأطوار الأربعة ومعيار خروجٍ مكتوبٍ لكلٍّ —
   > خاصةً الأخير: ما الذي يثبت أن القديم "يعجز عن الفعل"؟ أضفه إلى المستودع (repo).»*

الختام: *«أودع (commit) برسالة `02-2-object-storage-migration`.»*
...
> *و*صلاحيات القراءة/الكتابة للمستودع القديم مُلغاة، فلا مسارَ كود — أو إعدادٌ (config)
> منسيّ — يستطيع إحياءه.
...
مقابلٍ في البيانات الفعلية. لم يُضِف أحدٌ مليون مستخدم. وكانت مهمة النسخ (backup job) تُبلّغ
بالنجاح كل ليلة، رمزُ خروجٍ (exit code) صفر، بلا شكوى.

كان السبب مجلدًا لم يعرف سكربت النسخ (backup script) أنه تكاثر. كان السكربت *يستثني* بعنايةٍ مجلد
بناء الإطار (`.next`) — لكن إعادةَ هيكلةٍ (refactor) حديثة لنشرٍ أزرق-أخضر (4.2) أنشأت شقيقَين
...
المترجم (compiler) من *كلا* مجلدَي البناء كل ليلة، بينما ما يهم فعلًا — قاعدتا بيانات MongoDB
وحفنةُ ملفات إعداد (config files) — كسرٌ صغيرٌ من المجموع. الإصلاح سطرُ استثناءاتٍ واحد
...
تكرارٍ (replication) ليلًا، أمرَ حذفٍ ضد قاعدة البيانات (database) **الإنتاجية (production)** بدل النسخة المتماثلة (replica). ثم جاء
ما جعلها أسطورة: ذهبوا يستعيدون (restore) من النسخة الاحتياطية فاكتشفوا أن **آلياتهم الخمس**
للنسخ والتكرار كانت كلها تفشل بصمتٍ أو مُعدّةً خطأً (misconfigured). استعادوا في النهاية من لقطةٍ
يدويةٍ (manual snapshot) عمرها ست ساعاتٍ وُجدت بالحظ، وخسروا بعض البيانات — وبثّوا الاستعادة مباشرةً.
...
- **الحالة (State)** ما لا تستطيع توليده: قاعدة البيانات، رفوعات المستخدمين،
  الإعدادات والأسرار (secrets) التي تجعل *هذا* التنصيب (install) فريدًا. اخسرها تختفِ للأبد.
- **القطع الأثرية (Artifacts)** كلُّ ما تستطيع آلةٌ إعادةَ بنائه من الحالة + الكود:
  الحزم المترجمة (compiled bundles)، والكاشات (caches)، و`node_modules`، والكود العامل نفسه (يعيش في git).
...
— موجودةٌ وغائبةٌ معًا حتى تحاول الاستعادة فعلًا. تفشل النسخ بصمتٍ بعشر طرق: مسارٌ (path)
خاطئ، صلاحياتٌ منتهية (expired credentials)، تغييرُ مخططٍ (schema change) لا يعالجه سكربت الاستعادة، أرشيفٌ تالف (corrupt archive)، هيكلُ
...
|---|---|---|
| **ماذا** فيها؟ | «الخادم (server)» (فالكاشات إذًا) | «قواعد البيانات والرفوعات والإعدادات — لا شيء قابل للتوليد» |
...
*قصدتَ* حمايته. وهذا أصدق مع سكربتات التشغيل التي يكتبها الوكيل (agent)، فتبدو معقولةً وتعمل
...
1. **انسخ الحالة فقط.**
   ...
   > الإعداد — ويستثني صراحةً الكود المصدري وناتج البناء والكاشات والاعتماديات (dependencies).
...
3. **نفّذ تمرين الاستعادة.**
   > *«الآن استعِد من تلك النسخة وحدها إلى بيئةٍ (environment) جديدةٍ فارغة — بلا وصولٍ للأصل.
...
الختام: *«أودع (commit) برسالة `02-3-backups-and-restore-drill`.»*
...
> (`.next-*`، `*/cache/webpack`). أتمِت التمرين: مهمةٌ مجدولةٌ (scheduled job) تستعيد إلى قاعدةٍ
...
- **قاعدة النسخ 3-2-1** (موثّقة على نطاق واسع) — ثلاث نسخ، وسيطان، نسخةٌ خارج الموقع (offsite)؛ أرضيةُ عقلٍ جيدة.
...
**القنبلة الموقوتة.** كشف تدقيقٌ (audit) تعليقًا (comment) في نموذج بياناتٍ (data model) يقول فعليًا *«ينتهي بعد
...
المجموعات تستقبل نحو 3.65 مليون صفٍّ سنويًا وتحتفظ الآن *بكلها، للأبد*، على القرص (disk)
نفسه مع قاعدة البيانات الأساسية (primary database). لا شيء يحترق. لكن المسار كان امتلاءَ قرصٍ أو نفادَ
ذاكرةٍ (out-of-memory) يُسقِط بيانات التحليلات والمنتج الحيّ **معًا**، لأنهما يتشاركان جهازًا واحدًا.
وعد تعليقٌ بانتهاءٍ كفّ الكودُ بهدوءٍ عن احترامه.

**الوعد المكسور.** المنصةُ نفسها سمحت للمستخدمين بحذف حساباتهم. وكان ذلك يزيل وثيقةً (document)
واحدةً بالضبط — سجلّ المستخدم (user record) — ويترك بياناتٍ شخصيةً وسلوكيةً في نحو **ثلاث عشرة**
مجموعةً أخرى: جلسات (sessions)، سجل استماع، رموز أجهزة (device tokens)، رسائل، صفوف تحليلات. بينما وعدت سياسة
الخصوصية (privacy policy) بالمحو الكامل (full erasure). كان كود الحذف معرّفًا على *جدولٍ (table) واحد*؛ وبيانات المستخدم
تعيش عبر الرسم البياني (graph) كله. والفجوة بينهما مسؤوليةٌ قانونيةٌ (legal liability) ترتدي زيَّ ميزةٍ عاملة.
...
تحت «الخصوصية/الامتثال (privacy/compliance)»، لكن القنبلة تُظهره أولًا مشكلة **موثوقية (reliability)**: بياناتٌ تنمو بلا
حدٍّ تستنزف في النهاية القرص أو الذاكرة (memory)، وإن تشاركت جهازًا مع قاعدتك الأساسية أسقطت
المنتج معها. النمو غير المحدود انقطاعٌ (outage) على مؤقتٍ بطيء.

الخطأ المضاعِف كان **الاشتراك في الموطن** (Co-location): تحليلاتٌ خامٌ على المضيف (host)
نفسه مع القاعدة الحية، فتقرن مجالَي فشلهما (failure domains). حتى البيانات صحيحة الحجم أأمن حين لا
...
الانتهاء (expiry) بسذاجةٍ يحذف الصفوف الخام التي تعتمد عليها تلك الرسوم. التسلسل الآمن صارمٌ
ومرتّب:
...
> **لا تشغّل TTL حتى تكون كل قراءةٍ خارج النافذة مدعومةً بتجميعٍ (aggregate) نُقل خلفيًا (backfilled) *وتُحقّق
...
فخٌّ دقيقٌ وُجد في العمل نفسه: جمّع التجميعُ (rollup) الصفوفَ بوقت *بدء* الجلسة بينما قاس
الـTTL العمرَ من وقت *آخر* حدث. فجلسةٌ تمتد عبر منتصف الليل قد تنتهي قبل أن يلتقطها
التجميع. **يجب أن تعتمد مهمة الاحتفاظ (retention job) ومهمة التجميع (aggregation job) دلالةَ الوقت (time semantics) نفسها**، وإلا أسقطت
...
`deleteUserData()` يمشي على الرسم كله، موصولٌ في *كلا* الحذف الذاتي (self-service delete) وحذف المسؤول (admin delete) كي
لا ينحرفا. ويجب أن يكون *مختبَرًا* — تتالٍ يفوّت مجموعةً واحدة لا يُميَّز عن عاملٍ حتى
يجد تدقيقٌ (أو منظّم (regulator)) الصفوف المتبقية.
...
|---|---|---|---|
| أحداث تحليلات خام | 90 يومًا | تنقيح (debugging)، موثوقية | جمّعها إلى تجميعاتٍ يومية (daily aggregates) |
| تجميعات يومية | للأبد | اللوحات | — |
| الجلسات | 30 يومًا | أمن، حجم | احذف |
| بيانات مستخدمٍ محذوف | 0 (تتالٍ فوري) | وعد الخصوصية | حذفٌ صلبٌ (hard delete) عبر الرسم |
| التقاطات تشخيصية (diagnostic captures) | 30 يومًا | قد تحوي رموز استعادة (reset tokens) | أخفِ (redact) عند الاستقبال ثم أنهِ |
...
الختام: *«أودع (commit) برسالة `02-4-retention-and-erasure`.»*

> 🔧 **تحت الغطاء** (اختياري): في MongoDB الـTTL فهرسٌ (index) على حقل تاريخٍ بـ
...
> حذفٍ مرتّبةٌ موثّقة. وقاعدة «دلالة الوقت نفسها»: اختر حقلَ طابعٍ زمنيٍّ (timestamp field) واحدًا
...
- [ ] التجميع والـTTL يعتمدان — بإثبات — الطابعَ الزمني نفسه (لا خلطَ بدءٍ مقابل آخر
      حدث).
- [ ] حذفت مستخدمًا اختباريًا (test user) ثم مسحت كل مجموعةٍ ورأيت صفرَ صفوفٍ متبقية — محوٌ
      مُثبَتٌ لا مفترَض.
...
- **اللائحة GDPR المادة 17 "الحق في المحو (right to erasure)"** (gdpr-info.eu) — الشكل القانوني لوعد الحذف، مقروءًا.
...
طاردت الفرضياتُ الأولى ذُرى الحركة (traffic spikes) والنشرات السيئة (bad deploys). لم تدعم الرسومُ أيًّا منهما.
الحقيقة كانت صفحةً واحدة: **لوحة مسؤولٍ (admin dashboard) واحدة**، وكلما حمّلها أحد شغّلت **ثمانية مسوحٍ
غير محدودةٍ (unbounded scans) منفصلةٍ على أكبر مجموعةٍ (collection) في القاعدة** — قارئةً كل صفٍّ من البداية للنهاية،
ثماني مراتٍ، لتحسب رسومها.

وإليك لماذا أبطأ ذلك *الجميع* لا المسؤول وحده. تحتفظ قاعدة البيانات (database) ببياناتها كثيرة
...
الساخنة** ليفسح مكانًا. فصار على كل طلبٍ (request) عاديٍّ الرجوعُ إلى القرص لإعادة تحميل ما كان
فوريًا. لم تشغّل اللوحةُ استعلامًا بطيئًا (slow query) فحسب؛ بل أفرغت الكاش (cache) الذي يعتمد عليه المنتج
كله بصمت. تحميلُ صفحةٍ واحدة، ونطاقُ الأثر (blast radius): الجميع.

الإصلاح كان تحديدَ استعلامات اللوحة وتخزينَها المؤقت (caching) وإضافةَ الفهارس الصحيحة — لكن
...
بلا فهرسٍ (Index)، إيجادُ الصفوف المطابقة يعني **مسحًا كاملًا للمجموعة (full collection scan)** (في SQL:
*مسح تسلسلي (sequential scan)* أو COLLSCAN): تقرأ القاعدة *كل* صفٍّ وتفحص كلًّا. حسنٌ على 50 صفًّا؛
...
| في الخطة ترى… | يعني… |
|---|---|
| **مسح فهرس (index scan) / IXSCAN** | جيد — استعمل فهرسًا |
| **مسح تسلسلي / COLLSCAN** | قرأ الجدول كله — جريمة اللوحة |
| **صفوف مفحوصة (rows examined) ≫ صفوف معادة (rows returned)** | قرأ أكثر بكثيرٍ مما أبقى — فهرسٌ ناقص |
...
أشيع علة أداءٍ في كود الوكلاء (agent-written code): لعرض خلاصةٍ (feed) فيها 50 رفعًا مع منتجيها، يشغّل الكود
...
يجتاز N+1 كل اختبارٍ على 50 صفًّا ويموت على 5 ملايين — ولهذا يبقى إلى الإنتاج (production).
...
نسخةٍ متماثلة (replica) أو على تجميعات (rollups) — 2.4، 10.3)، وحدِّد وفهرس كل ما يلمس مجموعةً كبيرة.
...
| **وثائقية (document)** | سجلاتٌ مرنةٌ مستقلة | MongoDB | يتنوّع الشكل؛ قليلٌ من الوصلات (joins) بين الكيانات |
| **مفتاح-قيمة (key-value)** | بحثٌ سريعٌ بسيطٌ قابل للتخلص | Redis | كاش، جلسات (sessions)، عدّادات (الوحدة 3) |
...
1. **جِد المسارات البطيئة.**
   ...
   > واقعيًا — ابذر (seed) بضعة ملايين صفٍّ إن لزم. أشِر إلى أي مسحٍ كاملٍ للمجموعة.»*
...
الختام: *«أودع (commit) برسالة `02-5-indexes-and-queries`.»*
...
استغرق رفعٌ جماعيٌّ (bulk upload) لـ114 ملفًا من 15 إلى 30 دقيقةً لأنه لم يكن ممكنًا إلا **ملفًا
ملفًا**. ارفع اثنين معًا فتختفي ملفاتٌ بصمت — العدد في النهاية خاطئ، لكن لا خطأ يظهر.
وأشهرًا كان «الإصلاح» قاعدةً تُهمَس بين المسؤولين (admins): *ارفعها واحدًا واحدًا وإلا خسرت
...
الخادم (server) يفعل ثلاث خطواتٍ على وثيقةٍ (document) كبيرةٍ واحدة: **يقرأ** السجل (record)، **يضيف** الملف الجديد
...
الاختبار (testing) لسببٍ قاسٍ واحد: **اختبرت وحدك.** طلبٌ واحدٌ في كل مرةٍ لا يتشابك أبدًا.
لا توجد العلة إلا حين يحدث شيئان معًا — وهي كل لحظةٍ حقيقيةٍ في الإنتاج (production) ولا لحظةٍ على
جهازك.

الإصلاح الحقيقي لم يكن «ارفع تسلسليًا». بل جعلُ الكتابات **لا تتشارك سجلًا متغيّرًا
أصلًا**: كل ملفٍ يُرفع مباشرةً إلى التخزين (storage) تحت مفتاحه المستقل (عديم التسابق (raceless) —
...
- **العملية الذرّية (Atomic Operation)** — أفضل إصلاحٍ حين يناسب. بدل
  ...
  `$push` إلى مصفوفة (array)، `UPDATE … SET n = n + 1`. لا نافذة، لا تسابق. صار تأكيد حادثتنا
  إضافةً ذرّيةً واحدة.
...
- **القيد الفريد (Unique Constraint)** — خط الدفاع الأخير ضد التكرار. شخصان يسجّلان
  ...
  أيٌّ منهما — التسابق نفسه). قيدٌ `UNIQUE` في القاعدة يجعل الإدراج (insert) *الثاني* يفشل
...
- **مفتاح عدم تكرار الأثر (Idempotency Key)** — كي لا تتصرّف إعادةٌ مرتين. يرسل العميل (client)
  ...
  وإن رآه ثانيةً (نقرةٌ مزدوجة (double-click)، إعادةُ شبكة (network retry)، مستخدمٌ نافد الصبر) أعاد النتيجة الأولى بدل
...
يكتب الوكيل (agent) «اقرأ السجل، أضف العنصر، احفظه» لأنه أوضح تعبيرٍ عن القصد، ويُقرأ بلا عيب،
و**يجتاز كل اختبار — لأن الاختبارات تشغّل طلبًا واحدًا في كل مرة.** لا سبب للوكيل
أن يتخيّل عشرين نسخةً متزامنة ما لم تجبره. علل التزامن (concurrency bugs) لا تفشل بصخب؛ بل تفقد البيانات
...
الختام: *«أودع (commit) برسالة `02-6-races-and-idempotency`.»*
...
> `{ email: 1 }, { unique: true }`؛ ومفتاح عدم تكرار الأثر معرّفُ طلبٍ (request ID) مخزّنٌ يُفحَص

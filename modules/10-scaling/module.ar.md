...
stateDiagram-v2
    [*] --> مطابور
    مطابور --> قيد_التنفيذ: يطالب بها عامل (إيجار)
    قيد_التنفيذ --> منجَز: نجاح، إقرار
    قيد_التنفيذ --> مطابور: فشل، إعادة (محاولة < N)
    قيد_التنفيذ --> رسالة_ميتة: فشل N مرات
    قيد_التنفيذ --> مطابور: العامل تعطّل، انتهى الإيجار
    منجَز --> [*]
    رسالة_ميتة --> [*]: إنسان يحقق
```

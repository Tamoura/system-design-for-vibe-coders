...
*وكيلك (agent) يكتب الكود (code) أسرع مما تستطيع قراءته. تبني هذه الوحدة الشبكات الآلية (mechanical nets) التي تلتقط
أخطاءه دون أن تراقب كل ضغطة مفتاح: حواجز git، واختبارات (tests) تثبّت السلوك، وفحوص صادقة (checks) في
التكامل المستمر (CI)، والتحكم في الكود الذي لم تكتبه.*
...
في ظهيرة عادية، أنهى وكيلٌ (agent) ميزةً صغيرة وأنشأ إيداعًا (Commit). حذف الإيداع **أكثر من
600 ملف**.

لم يسبقه شيء درامي. تصادف أمران مملّان. أولًا، أُنشئ الفرع (Branch) من **الأساس الخطأ (wrong base)**
— فبدلًا من التفرّع من `main`، تفرّع من فرع ميزة (feature branch) قديم متأخر بمئات الملفات. ثانيًا، حين
حان وقت حفظ العمل، كان الأمر المستخدم `git add .` أي «جهّز (stage) كل شيء في هذا المجلد».
...
اكتُشف الأمر في المراجعة (review) هذه المرة. لكن تأمّل كم كان قريبًا. لم تكن ثمة رسالة خطأ (error message). ولا
شاشة حمراء. فأمرا `git add .` و`git commit` هما أكثر الأوامر اعتيادًا في العالم، وقد
عملا تمامًا كما صُمّما. لم تكن الكارثة عطلًا برمجيًا (bug) — بل أمرَين صحيحَين التقيا بافتراضٍ
واحد خاطئ بسرعة الآلة.

وهنا يقع ما يهمّك. لم يكن الرد **«فليكن الجميع أكثر حذرًا».** فالحذر ليس ضابطًا (control)، بل
أمنية. كان الرد مجموعةً من **القواعد الآلية (mechanical rules)** التي تعمل سواء تذكّرها أحد أم لا: خطّاف
(Hook) يرفض أي إيداع يمسّ أكثر من نحو 30 ملفًا، وخطّاف يرفض أي إيداع يحذف أكثر من بضعة
آلاف سطر، وقاعدة تتحقق من أساس الفرع (base branch) قبل بدء العمل. **بسرعة الوكيل، أي قاعدة أمان (safety rule) تسكن
...
تطلب من المشغّل (operator) أن يركّز أكثر. الجرّاحون يستعملون قوائم تحقق (checklists). والطيارون يستعملون قوائم
تحقق. لا لأنهم نسّاؤون، بل لأن الانتباه مورد محدود والفشل صامت. ووكيلك أسرع وأدأب من أي
مشغّل بشري، وبين يديه *صلاحياتك (permissions)*. فأي قاعدة يستطيع تجاوزها، سيتجاوزها في النهاية.

النسخة الشهيرة من هذا هي Knight Capital (قابلتها في F.3): نشرٌ (deploy) بلغ سبعة خوادم (servers) من ثمانية
وخسر 440 مليون دولار في 45 دقيقة. لم يكن أحد مهملًا. الناقص كان فحصًا آليًا (mechanical check) يتأكد أن
*كل* خادم يشغّل النسخة نفسها. وفي يوليو 2025، تجاهل وكيلٌ ذكي تعليمًا صريحًا بتجميد
الكود (code freeze) وحذف قاعدة بيانات إنتاجية (production database) — قاعدةٌ قيلت بالإنجليزية في محادثة ولم يحترمها الوكيل.
والنص في نافذة محادثة ليس حاجز أمان (guardrail). الخطّاف حاجز أمان.
...
(خطّاف *ما قبل الإيداع (pre-commit)*) أو قبيل إرساله الإيداعات إلى الخادم (خطّاف *ما قبل الدفع (pre-push)*).
...
|---|---|
| «تفرّع من الأساس الصحيح» | فحص بدء (startup check) يطبع أساس الفرع وهدف طلب الدمج (PR)؛ ويتوقف العمل إن اختلفا |
| «لا تجهّز كل شيء بلا تدقيق» | خطّاف يمنع الإيداعات فوق نحو 30 ملفًا ويطبع القائمة لتراها |
| «لا تحذف نصف المستودع (repo)» | خطّاف يمنع الإيداعات التي تزيل أكثر من نحو 5000 سطر |
| «انظر إلى ما جهّزته» | خطّاف يرفض المتابعة حتى تُعرض قائمة الملفات المجهّزة (staged files) |

### 3. العتبات تخمينات — ولا بأس بذلك

ثلاثون ملفًا، خمسة آلاف حذف — هذه أرقام اعتباطية عمدًا. فالمقصد ليس الدقة، بل **سلكٌ
كاشف (tripwire) عند حجمٍ غير طبيعي**. فإيداع الميزة العادي يمسّ حفنة ملفات. أما إيداعٌ يمسّ 600
فهو إما إعادة هيكلة (refactor) مشروعة تحدث مرة في السنة (تستحق تجاوزًا (override) مقصودًا) أو كارثة (تستحق
...
الحاجز بلا مخرجٍ للطوارئ (escape hatch) يُعطَّل بالكامل أول مرة يمنع فيها شيئًا مشروعًا — وحينها لا حاجز
...
وصريحًا بما يكفي كي يظهر في السجل (log). «ممنوع إلا إن كنت تقصد فعلًا» أفضل من «ممنوع دائمًا»
ومن «لا يُفحص أبدًا».
...
1. **ثبّت حارس عدد الملفات المجهّزة.**
   ...
   > يعيش في المستودع (مجلد `.githooks/`) كي يخضع للإصدارات (version-controlled)، وأخبرني بالأمر الوحيد الذي
   > أشغّله لتفعيله.»*
...
> 🔧 **تحت الغطاء** (اختياري): خطّاف ما قبل الإيداع سكربت تنفيذي (executable script) في `.githooks/pre-commit`؛
...
> مجلد الخطّافات الخاضع للإصدارات كي تنال نسخةٌ جديدة (fresh clone) الحمايةَ نفسها.

### سياق تعطيه للوكيل

- ما الأمر الوحيد الذي يفعّل الخطّافات في نسخة جديدة (كي يُحمى زميل أو جلسة وكيل (agent session) جديدة أيضًا).
...
كان في مشروعٍ **نحو 1990 اختبارًا ناجحًا (passing tests)**. أخضر بالكامل، في كل طلب دمج (PR)، لشهور. بدا
حصنًا.

ثم طرح تدقيقٌ (audit) سؤالًا وقحًا: *هل ستلتقط هذه الاختبارات استعلامًا معطوبًا (broken query) في قاعدة
البيانات؟* كان الجواب لا. فاستعلامٌ سيئ، أو فهرس (index) مفقود، أو ترحيل بيانات (Migration)
فاسد — كلٌّ منها سيمرّ عبر الحزمة (suite) كلها أخضرَ ولن ينكسر إلا أمام المستخدمين الحقيقيين.

كان السبب هادئًا وفي كل مكان. فأكثر اختبارات الخادم كانت **تحاكي قاعدة البيانات (Mock)**
— أي تستبدل طبقة البيانات (data layer) الحقيقية ببديلٍ يعيد ما يمليه عليه الاختبار. فيقول الاختبار
«حين نطلب المستخدم 42، تظاهرْ أن قاعدة البيانات تعيد هذا المستخدم»، ثم يستدعي الكود (code)، ثم
...
قيود التفرّد (unique constraints)، أو الحذف المتسلسل (cascade deletes) — وهي بالضبط ما ينكسر في الإنتاج (production).

وكانت ثمة عضّة ثانية. لأن المحاكاة رُبطت لتتوقع الاستدعاءات (calls) بترتيبٍ محدد، فإن إضافة
استعلام جديد واحد إلى دالة (function) كانت تكسر اختبارات في حزمٍ *غير ذات صلة* — اختبارات لا شأن
...
بيانات (database engine) حقيقي في الذاكرة (in-memory)** — قاعدة بيانات فعلية تعيش في الذاكرة طوال الاختبار،
سريعة بما يكفي للتكامل المستمر (CI)، وحقيقية بما يكفي لفرض القيود — زائد اختبار تكامل (integration test) واحد لكل
...
حين تخبر وكيلًا «أضف إعادة تعيين كلمة المرور (password reset)»، قد يفعلها جيدًا أو يقتطع زاويةً لن تلاحظها
...
المحاكاة (Mock) بديل زائف عن اعتمادٍ (dependency) حقيقي. وللمحاكاة استعمال مشروع: تزييف ما لا تملكه
ولا يمكنك استدعاؤه فعلًا في اختبار — مزوّد دفع (payment provider)، مرسِل بريد (email sender). والخطأ هو محاكاة الشيء الذي
*يدور عنه* الاختبار.
...
التغطية (coverage) الأكثر ليست أمانًا أكثر. فاختبارٌ يعيد فحص محاكاة، أو يؤكد أن دالة إرجاع (getter) تعيد ما
ضبطتَه، يضخّم رقمًا ولا يحمي شيئًا — **تغطية زائفة (vanity coverage)**. والاختبارات التي تكسب قيمتها تثبّت
ما ينكسر فعلًا:

| نوع الاختبار | ما يثبّته | لماذا يكسب قيمته |
|---|---|---|
| **كاشف الانجراف (drift guard)** | قائمتان يجب أن تبقيا متطابقتين (المسارات مقابل خريطة الموقع (sitemap)، الأعلام (flags) مقابل الواجهة) | الطريقة الأولى لتعفّن القوائم المتوازية (parallel lists) التي يديرها الوكيل (3.3، 6.4) |
| **اختبار العقد (contract test)** | شكل استجابة API يعتمد عليها كل عميل (client) | ينكسر عميل بصمت حين يُعاد تسمية حقل (field) |
...
اهدف إلى **هرم (pyramid)**: اختبارات وحدة (unit tests) كثيرة سريعة، اختبارات تكامل أقل، وحفنةٌ من الاختبارات
الشاملة البطيئة المبدئية. لا مخروط بوظة (ice-cream cone) من اختبارات واجهة (UI tests) بطيئة هشّة فوق قاعدة جوفاء.
...
كان خادم التطوير (development server) يصرّف (compiled) كل صفحة أول مرة تُزار، وكان مؤقّت الاختبار (timer) يَعُدّ ذلك التصريف
لمرة واحدة كأنه بطءٌ في التطبيق. أول زيارة: انتهاء مهلة (timeout) وبناء أحمر (red build). التشغيل الثاني: سليم.

الاختبار الذي ينذر كذبًا يدرّب الجميع على النقر «أعد التشغيل» بلا نظر — ويوم يلتقط خطأً
*حقيقيًا*، يعيدون تشغيله أيضًا. كان الإصلاح **تسخين المسارات مسبقًا (pre-warm routes)** قبل بدء الحزمة، كي
يقيس المؤقّت التطبيق لا المصرّف (compiler). وهذا ليس هامشًا؛ بل هو اللعبة كلها. **الحزمة غير الموثوقة
...
أحمر-أخضر-إعادة الهيكلة (red-green-refactor) — اكتب اختبارًا فاشلًا، اجعله ينجح، ثم نظّف — يُدرَّس عادةً
...
لدى Relay طبقة بيانات (حسابات، محتوى، خلاصات (feeds)) — وهي بالضبط حيث تنوّمك الاختبارات المحاكاة.
...
2. **حوّل أعلى الحزم خطرًا إلى محرك حقيقي.**
   > *«اختر أخطر حزم طبقة البيانات لدينا — التي تمسّ المال أو المصادقة (auth) أو الحذف. أعِد
...
4. **اقتل المتقلّبات (flakes) قبل أن تثق بالحزمة.**
...
5. **اجعل التكامل المستمر هو البوابة.**
   > *«اضبط التكامل المستمر كي يمنع اختبارٌ فاشل الدمجَ (merge) — لا تحذيرًا، بل منعًا. أرني طلب
...
6. **اكتب الذاكرة.** أضف إلى CLAUDE.md:
   ...
   > للأطراف الثالثة (third parties) التي لا نملكها. كل ترحيل ينال اختبار تكامل. الاختبارات المتقلّبة
...
> أو SQLite `:memory:` — عملية قاعدة بيانات (database process) فعلية تحادثها الاختبارات وتُفكَّك بعد كل
> تشغيل؛ وإصلاح التصريف على البارد (cold-compile) خطوة إعداد شاملة (global setup) تجلب كل مسار مرةً قبل بدء حزمة Playwright.
...
- التطوير الموجَّه بالاختبارات (TDD) مع وكيل = حدّد (أحمر) ← ابنِ (أخضر) ← نظّف (إعادة هيكلة)، والتكامل المستمر بوابةٌ تقدر أن تقول لا.
...
كان كل دفعٍ إلى المشروع يشغّل **فحصًا مسبقًا (preflight)** — سكربتًا (script) سريعًا في التكامل
المستمر (CI) يهدف إلى التقاط الأخطاء الواضحة قبل البناء (build) الحقيقي. وكل دفعٍ، كان أحد فحوصه
**يفشل**: فحص ملف القفل (lockfile) افترض مدير الحِزَم (package manager) الخطأ. فالمستودع يستعمل npm،
...
العلامة الحمراء بجانب كل إيداع (commit) كخلفية الجدار — حاضرة دائمًا، لا تعني شيئًا. وتوقّف الناس
عن قراءة مخرجات (output) الفحص المسبق تمامًا، لأنها 99 مرة من 100 كانت الإنذار الكاذب (false alarm) المعروف.
...
تحتاجه الإشارة (signal) الحقيقية ويدرّب ردّ الفعل ذاته — «ادفع عبر الأحمر» — الذي يسمح بمرور فشلٍ حقيقي.
...
إنذارات اسمٌ لهذا، **إرهاق الإنذار (alarm fatigue)**، ويعامل الإنذار الكاذب المزمن كعطلٍ *يُزال* لا
...
التدريب الخاطئ. وإبقاء الكاذب في مكانه هو المسار الوحيد المفضي إلى *الاختراق (breach)* — الفشل
...
| الحالة | الحكم |
|---|---|
| الفحص صائب ويبوّب (gates) البناء | ✅ أبقِه — هذا الهدف |
| الفحص خطأ / ضوضائي (noisy) | 🔧 أصلحه *اليوم*، أو 🗑️ احذفه *اليوم* |
...
متضادين. فاختبار قبول (acceptance test) أي فحص برهانان لا واحد:

- **أثبت أنه يستطيع الفشل:** أدخِل عمدًا المشكلة نفسها التي يهدف لالتقاطها، وشاهده يصير
  أحمر. (حزمة شاملة (end-to-end suite) تعمل فقط بتشغيلٍ يدوي مع «تابع عند الخطأ» — وجد التدقيق واحدة —
  *لا تستطيع* إفشال البناء. إنها زينة (decoration).)
- **أثبت أنه يستطيع النجاح:** أصلح المشكلة وشاهده يصير أخضر.

الفحص الذي فعل الاثنين، أمامك، هو وحده الذي كسب مكانًا في البوابة (gate).

### 4. الكذب المتكرر وعدم القدرة على الفشل مرضٌ واحد

اختبارات الدرس 8.2 المتقلّبة (flaky) والفحص المسبق الدائم الفشل في هذا الدرس يبدوان مختلفين لكن
...
1. **أضف فحصًا مسبقًا مقتصرًا على فحوص حقيقية سريعة.**
   ...
   > الأنواع (type check)، واختبار مبدئي (smoke test) أن التطبيق يقلع. أبقِه تحت الدقيقة. اسرد بالضبط ما يتحقق منه
   > كل فحص وماذا يعني فشل كلٍّ منها فعلًا.»*
2. **أثبت أن كل فحص يستطيع الفشل.**
   > *«لكل فحص مسبق، أدخِل — في فرع مؤقت (temporary branch) — المشكلة نفسها التي يهدف لالتقاطها، وأرني ذلك
...
> الصياغة، أودِع خطأ صياغة متعمَّدًا وشاهد المهمة (job) تصير حمراء؛ ولاختبار الإقلاع المبدئي،
> اكسر استيرادًا (import)؛ والحزمة المعلَّمة `continue-on-error: true` في سير عمل GitHub Actions
> تبلّغ عن الإخفاقات لكنها لا تضبط حالة خروج (exit status) المهمة، فلا تستطيع منع دمج أبدًا.
...
- Google SRE Book، **«Monitoring Distributed Systems»** (sre.google، مجاني) — الفصل عن سبب تدمير الإنذارات الضوضائية للثقة؛ التكامل المستمر مراقبةٌ (monitoring) لكودك.
...
في 22 مارس 2016، دخل مطوّر (developer) في نزاعٍ حول تسمية، واحتجاجًا **ألغى نشر (unpublished)** حِزَمه (packages) على npm.
كانت إحداها `left-pad`: **أحد عشر سطرًا** تحشو نصًّا بمسافات على اليسار. تافهة. لكنها،
كما تبيّن، معتمَدٌ عليها — بشكلٍ غير مباشر — من شريحة ضخمة من عالم JavaScript، تشمل أطرًا (frameworks)
كبرى. وخلال دقائق، انكسرت عمليات البناء (builds) *عبر الصناعة كلها*. شركات لم تسمع بـ`left-pad`
...
ولبنكنا النسخة الأحلك والأهدأ. وجد تدقيقٌ أن خط التكامل المستمر والنشر التلقائي (CI and auto-deploy pipeline) كله يعمل
على **جهاز Mac شخصي** يحمل مفاتيح النشر (deploy keys) عبر SSH ويشغّل **كودًا غير موثوق (untrusted code) من طلبات الدمج (pull requests)**
— أي أن أي أحدٍ يفتح طلب دمج يستطيع تشغيل كودٍ على الجهاز الذي يستطيع النشر إلى الإنتاج (production).
وكانت إجراءات البناء الطرفية (third-party build actions) التي يستعملها مثبَّتة بالوسم (tag) فقط (لصيقةٌ متحركة)، لا
ببصمةٍ ثابتة (fixed fingerprint) — فمهاجمٌ (attacker) يخترق أحد تلك الإجراءات يستطيع دفع كودٍ خبيث (malicious code) يتدفّق مباشرةً إلى
البناء تحت اسمٍ يبدو دون تغيير. لا انقطاع (outage) بأسلوب `left-pad` بعد. مجرد مسدّس محشوّ على
الطاولة، مصوَّب نحو الإنتاج، لن ترصده أي مراجعة كودٍ (code review) واحدة — لأن الخطر لم يكن في *كودنا* أصلًا.
...
الحِزَم التي تنتقيها هي القمة. تحتها **الشجرة غير المباشرة (transitive)** — اعتمادياتها (dependencies)،
*واعتماديات* اعتمادياتها، عمقها مئاتٌ غالبًا — كودٌ لم تختره، من أناسٍ لن تلقاهم، يعمل
بالصلاحيات (permissions) نفسها لكودك. زائد **إجراءات البناء**: الخطوات الطرفية التي يشغّلها تكاملك
...
|---|---|---|
| **التصيّد الإملائي (typosquat)** | `expres`، `lodahs` — حرف واحد عن الحزمة الحقيقية | يخطئ وكيلك اسمًا؛ فتثبّت البرمجية الخبيثة (malware) نفسها |
| **صاحب مهجور / مُختطَف (abandoned / hijacked maintainer)** | يعتزل صاحب حزمة شائعة، أو يسلّم المفاتيح لغريبٍ يشحن خبيثًا في تحديث «صغير» | تحدّث تلقائيًا إلى داخل الحمولة (payload) |
| **ثغرة معروفة (CVE) عميقة في الشجرة** | ثغرة موثَّقة في حزمةٍ خمسة مستويات تحت | أنت مكشوف عبر كودٍ لم تعلم أنك تملكه |
| **سكربت تثبيت خبيث (malicious install script)** | حزمة تشغّل سكربتًا *لحظة تثبيتها* — قبل أن تشغّل منها سطرًا | اختراقٌ (compromise) وقت التثبيت لا وقت التشغيل |
...
لا يعني هذا كتابة كل شيء بنفسك (قابلت «التقنية المملّة (boring technology) واشترِ السلعة (commodity)» في 0.3 — الاعتماديات
هي كيف تشتريها). بل يعني معاملة الشجرة بنيةً (infrastructure) *تحكمها*:
...
2. **شغّل بوابة تدقيق (audit gate) في التكامل المستمر.** التدقيق (audit) يقارن شجرتك بقاعدة بيانات
   ثغراتٍ معروفة (known vulnerabilities). اربطه بوابةً **تُفشل البناء عند ثغرةٍ حرجة (critical vulnerability) معروفة** — مُثبَتة القدرة
...
4. **اعزل (isolate) كل ما يشغّل كودًا غير موثوق.** التكامل المستمر الذي يشغّل كود طلبات الدمج يجب
   ألا يحمل مفاتيح الإنتاج على جهاز شخصي. التنفيذ غير الموثوق وبيانات اعتماد النشر (deployment credentials) لا
   يتشاركان مضيفًا (host) أبدًا.

### 4. قاعدة الوكيل: كل حزمة جديدة تحتاج تبريرًا من سطر وموافقتك

وهنا السلوك الذي يجعل هذا درسًا *للمبرمج بالوصف (vibe-coder)* لا مجرد درسٍ أمني: **الوكلاء يثبّتون
كل ما يُزيل الخطأ.** دالة مفقودة؟ غريزة الوكيل أن يثبّت حزمةً توفّرها — أسرع طريق من
الأحمر إلى الأخضر. لن يوازن هل الحزمة مصونة (maintained) أو شائعة أو تصيّدٌ إملائي. ولن يلحظ أنها
...
سلامةٍ (sanity check) من ستين ثانية، وموافقتك الصريحة.**
...
فحص السلامة ستون ثانية فعلًا: أهي واسعة الاستعمال (التحميلات (downloads))؟ أمصونة (آخر إصدار هذا
...
1. **ابنِ سجلّ الاعتماديات (dependency ledger).**
   > *«اسرد كل اعتمادية مباشرة (direct dependency) في Relay. ولكلٍّ، أعطني سطرًا: لماذا نحتاجها، وعدد
...
3. **أضف بوابة التدقيق — وأثبت أنها تطلق.**
   > *«أضف بوابة تكامل مستمر تشغّل تدقيق اعتماديات (dependency audit) وتُفشل البناء عند أي ثغرة حرجة معروفة.
...
> الانحراف (drift) عن القفل)؛ والتثبيت بالبصمة يستبدل `uses: some/action@v3` بـ
...
- `left-pad`: أحد عشر سطرًا، غريبٌ واحد، وصناعةٌ من عمليات بناءٍ مكسورة — سلسلة التوريد *هي* المعمارية (architecture).

## 📚 المراجع ومزيد من القراءة

- **حادثة left-pad** (مارس 2016) — مقال Quartz «How one programmer broke the internet…» وردّ npm؛ الأسطر الأحد عشر نفسها تستحق النظر.
- **OWASP Top 10: A06 المكوّنات الضعيفة والقديمة (Vulnerable and Outdated Components)** (owasp.org) — خطر سلسلة التوريد في القائمة المعيارية للصناعة (نعود إليها في 5.6).
- توثيق **npm audit** / **GitHub Dependabot** — أدوات التدقيق المجانية التي يربطها هذا الدرس بالتكامل المستمر.
- **إطار SLSA** (slsa.dev) — النموذج الحالي لسلامة البناء (build integrity) وسلسلة التوريد، ولماذا تثبّت بالبصمة لا بالوسم.
